    ...
    for l in range(1, L + 1):
        ...
        for i in range(len(A1)):
            for j in range(len(A1[0])):
                A1[i,j]=sigmoid(activation[f"Z*{l}"][i, j])
        activation[f"A{l}"] = A1  
    return activation
...
base_path=r"C:\Users\HP\Desktop\D folder\Linu_savitha_2\minibatch_pgm\Batch_N\output_sigmoid_cifar"
...
V = T.tolist()
with open(f"{base_path}\\sigmoid_prediction_cifar.csv","w", newline='') as file:
