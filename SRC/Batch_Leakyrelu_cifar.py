        ...
        for i in range(len(A1)):
            for j in range(len(A1[0])):
                if activation[f"Z*{l}"][i, j] >= 0:
                    ...
                    A1[i, j] = alpha*activation[f"Z*{l}"][i, j]       
        activation[f"A{l}"] = A1  
    return activation
...
epoch =10
batch_size = 1000
min_delta = 1e-2  # Minimum change in loss to count as an improvement
...
base_path=r"C:\Users\HP\Desktop\D folder\Linu_savitha_2\minibatch_pgm\Batch_N\output_leakyrelu_cifar"
vali = np.array(val_losses)
with open(f"{base_path}\\val_loss__drybean.csv", "w", newline='') as file:
    csv.writer(file).writerows([[v] for v in vali.tolist()])

#------Saving the prediction values------------------
T = np.hstack((test_Y.T, prediction.T))
V = T.tolist()
with open(f"{base_path}\\Leakyrelu_prediction_drybean.csv","w", newline='') as file:
...
precision_macro = precision_score(true_labels, pre_labels, average='macro',zero_division=1)
recall_macro = recall_score(true_labels, pre_labels, average='macro',zero_division=1)

# Micro-average
# Micro-average with zero_division set to 1 (handle undefined cases)
precision_micro = precision_score(true_labels, pre_labels, average='micro',zero_division=1)
recall_micro = recall_score(true_labels, pre_labels, average='micro',zero_division=1)
...
print(classification_report(true_labels, pre_labels, digits=4,zero_division=1))
# F1 Macro
f1_macro = f1_score(true_labels, pre_labels, average='macro',zero_division=1)

# F1 Micro
f1_micro = f1_score(true_labels, pre_labels, average='micro',zero_division=1)

print(f"F1 Macro: {f1_macro:.4f}")
print(f"F1 Micro: {f1_micro:.4f}")    

f1_weighted = f1_score(true_labels, pre_labels, average='weighted',zero_division=1)
