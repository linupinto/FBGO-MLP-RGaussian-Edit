...
def backwardPropagation(X, Y, parameters, activation):
    ...
    C1 = np.exp(-1 * activation[f"Z{L-1}"])
    B1=(1-activation[f"A{L-1}"])/(1+C1)
    change[f"dA{L - 1}"] = activation[f"A{L-1}"]+B1
    ...
    for l in range(L - 2, 0, -1):
        C1 = np.exp(-1 * activation[f"Z{l}"])
        B1=(1-activation[f"A{l}"])/(1+C1)
        change[f"dA{l}"] = activation[f"A{l}"]+B1
...
base_path=r"C:\Users\HP\Desktop\D folder\Linu_savitha_2\minibatch_pgm\minibatch\output_swish_cifar"
...
V = T.tolist()
with open(f"{base_path}\\swish_prediction_cifar.csv","w", newline='') as file:
