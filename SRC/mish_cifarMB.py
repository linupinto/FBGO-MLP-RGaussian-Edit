...
def backwardPropagation(X, Y, parameters, activation):
    ...
    change[f"dA{L - 1}"]=mish_derivative(activation[f"Z{L-1}"])
    ...
    for l in range(L - 2, 0, -1):
        ...
        change[f"dA{l}"]=mish_derivative(activation[f"Z{l}"])
...
base_path=r"C:\Users\HP\Desktop\D folder\Linu_savitha_2\minibatch_pgm\minibatch\output_mish_cifar"
...
V = T.tolist()
with open(f"{base_path}\\mish_prediction_cifar.csv","w", newline='') as file:
