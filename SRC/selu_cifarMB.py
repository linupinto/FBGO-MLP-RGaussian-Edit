...
def selu(x):
    return lambda_selu * np.where(x > 0, x, alpha_selu * (np.exp(x) - 1))

def selu_derivative(x):
    return lambda_selu * np.where(x > 0, 1, alpha_selu * np.exp(x))
...
def backwardPropagation(X, Y, parameters, activation):
    ...
    change[f"dA{L - 1}"]=selu_derivative(activation[f"Z{L-1}"])  # GELU DERIVATIVE
    ...
    for l in range(L - 2, 0, -1):
        change[f"dA{l}"]=selu_derivative(activation[f"Z{l}"])
...
base_path=r"C:\Users\HP\Desktop\D folder\Linu_savitha_2\minibatch_pgm\minibatch\output_selu_cifar"
...
V = T.tolist()
with open(f"{base_path}\\selu_prediction_cifar.csv","w", newline='') as file:
