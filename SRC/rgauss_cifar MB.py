...
for l in range(1, L + 1):
    ...
    cvalues[f"c{l}"] =np.zeros(( 1,layer_dims[l]))
    
#parameters[f"W{L}"] = np.random.uniform(1, 2,(layer_dims[L], layer_dims[L - 1])) # last layer weights
for l in range(1, L):
    activation[f"A{0}"] = train_X    
...
def forwardPropagation(X, Y, parameters):
    ...
    for l in range(1, L + 1):
        ...
        activation[f"Z{l}"] = Z
        Max = np.matrix(np.max(activation[f"Z{l}"], axis=0))
        Min = np.matrix(np.min(activation[f"Z{l}"], axis=0))
        CL = Max - Min
        M = np.repeat(Max, m, axis=0)
        C = np.repeat(CL, m, axis=0)
        C1 = 2 * np.square(C)
        N = -1 * np.square(activation[f"Z{l}"] - M)
        A1 = np.exp(N / C1)
        activation[f"A{l}"] = A1
        cvalues[f"c{l}"] =CL
    return activation
...
def backwardPropagation(X, Y, parameters, activation):
    ...
    Max = np.matrix(np.max(activation[f"Z{L - 1}"], axis=0))
    Min = np.matrix(np.min(activation[f"Z{L - 1}"], axis=0))
    CL = Max - Min
    M = np.repeat(Max, m, axis=0)
    C = np.repeat(CL, m, axis=0)
    change[f"dA{L - 1}"] = np.divide(np.multiply(activation[f"A{L - 1}"], -(activation[f"Z{L - 1}"] - M)), np.square(C))
...
base_path=r"C:\Users\HP\Desktop\D folder\Linu_savitha_2\minibatch_pgm\minibatch\output_rgauss_cifar_minibatch"
...
V = T.tolist()
with open(f"{base_path}\\rgauss_prediction_cifar.csv","w", newline='') as file:
