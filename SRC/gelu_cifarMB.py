...
def gelu_derivative(x):
    ...
    return 0.5 * (1 + erf_term) + (x * exp_term) / sqrt_2pi
...
def backwardPropagation(X, Y, parameters, activation):
    ...
    change[f"dA{L - 1}"]=gelu_derivative(activation[f"Z{L-1}"])  # GELU DERIVATIVE
    ...
    for l in range(L - 2, 0, -1):
        change[f"dA{l}"]=gelu_derivative(activation[f"Z{l}"])
...
base_path=r"C:\Users\HP\Desktop\D folder\Linu_savitha_2\minibatch_pgm\minibatch\output_gelu_cifar"
