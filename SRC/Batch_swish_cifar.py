    ...
    for l in range(1, L + 1):
        ...
        for i in range(len(A1)):
            for j in range(len(A1[0])):
                A1[i,j]=activation[f"Z*{l}"][i, j]*sigmoid(activation[f"Z*{l}"][i, j])
        activation[f"A{l}"] = A1  
    return activation
    ...
    for l in range(L-1, 0,-1): # l t0 1   
        ...
        for i in range(len(B)):
            for j in range(len(B[0])):
                x=sigmoid(activation[f"Z*{l}"][i, j])
                B[i,j]=x*(1+activation[f"Z*{l}"][i, j]*(1-x))
        change[f"dA{l}"] = B
        #return change
...
base_path=r"C:\Users\HP\Desktop\D folder\Linu_savitha_2\minibatch_pgm\Batch_N\output_swish_cifar"
