...
from confmat import confmat_CIFAR,confmat_Drybeans
...
def forwardPropagation(X, Y, parameters):
    ...
    for l in range(1, L + 1):
        ...
        A1=leaky_relu(activation[f"Z{l}"])
        activation[f"A{l}"] = A1
    return activation
...
def backwardPropagation(X, Y, parameters, activation):
    ...
    change[f"dA{L - 1}"]=leaky_relu_derivative(activation[f"Z{L-1}"])  
    ...
    for l in range(L - 2, 0, -1):
        change[f"dA{l}"]=leaky_relu_derivative(activation[f"Z{l}"])
...
base_path=r"C:\Users\HP\Desktop\D folder\Linu_savitha_2\minibatch_pgm\minibatch\output_leakyrelu_cifar"
...
V = T.tolist()
with open(f"{base_path}\\leakyrelu_prediction_cifar.csv","w", newline='') as file:
