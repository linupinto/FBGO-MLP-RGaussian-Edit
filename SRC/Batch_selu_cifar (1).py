        ...
        for i in range(len(A1)):
            for j in range(len(A1[0])):
                if activation[f"Z*{l}"][i, j] > 0:
                    A1[i, j] = lambd*activation[f"Z*{l}"][i, j]
                else:
                    A1[i, j] =  lambd*alpha*(exp(activation[f"Z*{l}"][i, j])-1)      
        activation[f"A{l}"] = A1  
    return activation
        ...
        for i in range(len(B)):
            for j in range(len(B[0])):
                if activation[f"Z*{l}"][i, j] > 0:
                    ...
                    B[i, j] = lambd*alpha*exp(activation[f"Z*{l}"][i, j])
        change[f"dA{l}"] = B
        #return change
...
base_path=r"C:\Users\HP\Desktop\D folder\Linu_savitha_2\minibatch_pgm\Batch_N\output_selu_cifar"
